...
export class UsersComponent implements OnInit{
    ...
    sendUser(): boolean {
        ...
        this.http.post("spacelama-api:9000/api/user", {
    ...
    deleteUser(user: User) {
        ...
        this.http.delete("spacelama-api:9000/api/user/" + user.id).toPromise().then(response => {
            console.log(response);
            this.users.splice(this.users.indexOf(user), 1);
        });
    }
    getAllUsers() {
        this.http.get('spacelama-api:9000/api/user').toPromise().then(users => {
            console.log(users);
            let i;
            this.users = [];
            for (i in users) {
